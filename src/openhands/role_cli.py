...
from .role import DEFAULT_SUMMARY_INSTRUCTIONS, run_role_with_summary
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="openhands-role",
        description=(
            "Run an OpenHands role conversation, then repeatedly ask OpenHands "
            "to summarize the answer until the summary is valid JSON. Prints both."
        ),
    )
    parser.add_argument(
        "--endpoint",
    ...
    parser.add_argument(
        "--summary-model",
        default=None,
        help="Optional model for the summary conversation. Defaults to --model when provided; otherwise omitted.",
    )
    parser.add_argument(
        "--summary-instructions",
        default=None,
        help="Custom instructions for producing the summary JSON. Replaces the built-in role summary schema prompt.",
    )
    parser.add_argument(
        "--summary-instructions-file",
        default=None,
        metavar="PATH",
        help="Read custom summary JSON instructions from file. Overrides --summary-instructions.",
    )
    parser.add_argument(
        "--summary-max-attempts",
        type=int,
        default=0,
        help="Maximum JSON-summary attempts. 0 means retry until valid JSON. Default: 0.",
    )
    parser.add_argument(
        "--summary-show-attempts",
        action="store_true",
        help="Print summary retry attempt numbers and JSON parse errors to stderr.",
    )
    parser.add_argument(
        "--output-json",
        action="store_true",
        help="Print one machine-readable JSON object containing answer, summary, and metadata.",
    )
    parser.add_argument(
        "--answer-file",
        default=None,
        metavar="PATH",
        help="Also write the full role answer to this file.",
    )
    parser.add_argument(
        "--summary-file",
        default=None,
        metavar="PATH",
        help="Also write the validated pretty-printed summary JSON to this file.",
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    try:
        if args.summary_instructions_file:
            summary_instructions = Path(args.summary_instructions_file).read_text(encoding="utf-8")
        elif args.summary_instructions:
            summary_instructions = args.summary_instructions
        else:
        ...
        result = asyncio.run(
            run_role_with_summary(
                ...
                summary_show_attempts=args.summary_show_attempts,
        ...
        print(f"openhands-role: {exc}", file=sys.stderr)
        raise SystemExit(1) from exc
    except KeyboardInterrupt:
        raise SystemExit(130)

    summary_pretty = json.dumps(result.summary_json.model_dump(mode="json"), ensure_ascii=False, indent=2, sort_keys=True)

    if args.answer_file:
        Path(args.answer_file).write_text(result.answer + "\n", encoding="utf-8")
    if args.summary_file:
        Path(args.summary_file).write_text(summary_pretty + "\n", encoding="utf-8")

    if args.output_json:
        print(
            json.dumps(
                result.to_dict(include_answer=True, include_raw_summary=True, include_attempt_text=False),
                ensure_ascii=False,
                indent=2,
                sort_keys=True,
            ),
        ...
        print("========== OpenHands answer ==========")
        print(result.answer)
        print("========== OpenHands summary JSON ==========")
